...
def sentiment_analysis(keyword, language, file_name='', live=False):
    ...
    plot1.bar(sentiment, score, color=((0.3, 0.8, 0.3, 0.8), (0.8, 0.3, 0.3, 0.8)))
    plot1.set_title(f"Are people on Twitter positive or negative about {keyword}?")
    ...
    font = FontProperties()
    font.set_size('small')
    plot2.text(0, 6, f"Most positive tweet:\n{wrap_text(most_positive_tweet, 100)}", fontproperties=font)
